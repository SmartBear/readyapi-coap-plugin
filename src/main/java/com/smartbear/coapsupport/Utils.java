...
public class Utils {
    ...
    public static byte[] hexStringToBytes(String str) {
        ...
        if (str.length() % 2 != 0) {
            throw new IllegalArgumentException("The string has odd number of characters which correspond to hexadecimal digits so it cannot be interpreted as a byte sequence.");
        }
    ...
    public static String limitStringLen(String s, int maxLength) {
        ...
        int pos = s.indexOf("\n");
        if (pos != -1 && pos < newLen) {
            newLen = Math.min(pos, maxLength - 3);
        }
        if (newLen == s.length()) {
            return s;
        } else {
